...
def get_labels(computed_test_outputs, test_outputs, label_names):
    from itertools import permutations
    from sklearn.metrics import accuracy_score
    all_permutations = list(permutations(label_names))
    all_accuracies = []
    for i in range(len(all_permutations)):
        labeled_computed_test_outputs = [all_permutations[i][value] for value in computed_test_outputs]
        all_accuracies.append(accuracy_score(test_outputs, labeled_computed_test_outputs))
    return all_permutations[all_accuracies.index(max(all_accuracies))]
